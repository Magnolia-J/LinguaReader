...
- **修改功能**
  - 「打开即续读」滚动恢复时序修正：`init()` 中字号/行距/护眼偏好 `applyReaderPrefs()` 从「续读打开书之后」提前到「打开书之前」，确保阅读区首次渲染即用正确布局；`renderReadingArea` 的滚动恢复由单 `requestAnimationFrame` 改为**双 `requestAnimationFrame`**，等浏览器完成布局（`scrollHeight` 就绪）后再设 `scrollTop`，避免大数值被夹到顶部。
  - 「上次读到这里」标记显示时序修正：有滚动可恢复时，在双 rAF 内**先恢复滚动、再显示标记**（锚点=真实滚动位置），避免先显示标记（锚点=旧滚动 0）后大滚动触发淡出逻辑误判为「已离开」而隐藏；`onReadingScroll` 淡出判定加 `!_restoring` 守卫，恢复期间不误触发淡出。
- **删除功能**：无。
- **Bug 修复**
  - 续读滚动位置错位（调过字号/行距的书重开后正文滚回顶部/靠前）：根因为 `applyReaderPrefs` 在续读之后才应用，字号改大使内容顶高、像素滚动量对应更靠前位置（标记按 `offsetTop` 重排后计算故位置对、正文却错）。
  - 续读后「上次读到这里」标记消失：根因为双 rAF 改动使标记在第 1 帧显示（锚点记旧滚动 0）、第 2 帧大滚动恢复触发 `onReadingScroll` 判定偏离 >60px 而加 `faded`（`opacity:0`）。
- **数据库变更**：无。
- **API 变更**：无。
- **Prompt 变更**：无。
- **文档一致性**：四份文档统一到 v1.5.0；续读/返回阅读位置行为在 README/PRD/TECHNICAL_DESIGN/BLUEPRINT 同步修正；标注 `package.json` 的 `version` 仍为 1.2.0，与文档版本不一致（**需人工补充**：建议将 `package.json` 升至 1.5.0）。

### 📝 Update Summary（2026-08-17 · v1.4.0）

- **新增功能**
  - **返回原阅读位置（Return-to-Reading）**：在「知识库 → 原文定位」「批注定位」等跳转场景中，首次进入时自动冻结「进入前精确阅读位置 A」（章节 + 段落 + 滚动 + 文本指纹），阅读区顶部出现「↩ 返回阅读位置」浮层按钮；点按精确回到 A 并清除临时状态。该临时位置与正常阅读进度 `progress` **完全分离**（独立 `localStorage` 键 `lr_temp_return`，不进 `store.json`、不进云端同步），临时跳转**绝不覆盖**正常阅读进度；连续跳转不覆盖 A；主动阅读（滚动/切章）后不强制返回；切书 / 切章 / 关闭页面自动清除临时状态。
  - **笔记删除墓碑同步（`deletedKbIds`）**：与书籍墓碑 `deletedBookIds` 同款。在任一端删除一条笔记 / 清空知识库时，把被删 id 追加进本地 `store.deletedKbIds`，经 `PUT /api/store` 同步到云端与后端；合并时按 id 并集过滤已删笔记，彻底解决「本地删笔记 → 云端/手机端复活」问题（根因是 `sync.js` 的 union 合并曾让本地删除的笔记从云端复活）。
  - **一键重启脚本 `restart_lr.bat`**：双击即结束占用 `3007` 的进程并重新前台启动 `server.js`（弹黑框），供休眠/重启后服务未连接时手动快速恢复。
  - **任务计划程序自动重启**：新增计划任务 `LinguaReader_Service_Restart`，触发器为「工作站解锁时」运行 `restart_lr.bat`，解决笔记本合盖休眠/睡眠恢复后启动文件夹自启项不重跑、服务进程被系统清理导致的「服务未连接」。
- **修改功能**
  - 启动 / 自启方案调整：启动文件夹 `LinguaReader_AutoStart.bat`（位于 `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup`）改为**直接前台运行 Node**（弹黑框，稳定可用）；`start_silent.bat` 改为调用 `run_server_hidden.vbs`（无窗口启动器，但当前存在引号拼接 bug 已暂弃用，回退为弹黑框方案）。
  - `PUT /api/store` 合并逻辑增强：在原有 `deletedBookIds` 书籍墓碑基础上，新增 `deletedKbIds` 笔记墓碑（`kbTomb`），与书籍同理做并集合并 + 安全守卫（仅移除 `body.kb` 已省略的笔记，防止 tombstone 被污染误删全部笔记）。
- **删除功能**：无（PDF 解析此前已彻底移除；`run_server_hidden.vbs` 无窗口方案因 bug 暂弃用但文件保留）。
- **Bug 修复**
  - 笔记「删除后在云端/手机端复活」：根因 `sync.js` 的 union 合并让本地删除的笔记从云端复活；修复 = `deletedKbIds` 墓碑 + `server.js` 的 `kbTomb` 守卫 + `sync.js` 本地写回护栏（保留「用户已恢复、但仍处于墓碑中的笔记」，云端 payload 仍被过滤）。
  - 休眠/睡眠恢复后「服务未连接」：根因休眠恢复不触发启动文件夹自启项、server 进程被系统清理；缓解 = `restart_lr.bat` 一键恢复 + 任务计划「工作站解锁时」自动重启（待用户侧实测验证）。
- **数据库变更**：本地 `data/store.json` 顶层新增 `deletedKbIds`（笔记墓碑数组，随 store 同步云端与后端）；浏览器 `localStorage` 新增 `lr_temp_return`（返回阅读位置临时状态，非 `store.json`、非云端）；云端 `kb_store` 无结构变化（payload 内新增 `deletedKbIds` 字段）。
- **API 变更**：无新增端点。`markKbDeleted` / `clearKb` 复用 `PUT /api/store`（携带 `deletedKbIds`）/ `DELETE /api/kb`；返回阅读位置无专属 API（纯 `localStorage`）。`PUT /api/store` 由「仅书籍 tombstone 合并」扩展为「书籍 + 笔记双 tombstone 合并」。
- **Prompt 变更**：无。
- **文档一致性**：四份文档统一到 v1.4.0；功能表 / 数据模型 / 页面结构 / 映射 / Roadmap 同步覆盖「返回原阅读位置」「笔记墓碑」「自启与休眠恢复」；标注 `package.json` 的 `version` 仍为 `1.2.0`，与文档版本不一致（**需人工补充**：建议将 `package.json` 升至 `1.5.0`）。

### 📝 Update Summary（2026-08-10 · v1.3.0）

- **新增功能**
  - **阅读欢迎 & 读完纪念（阅读仪式）**：打开工作台时随机弹出欢迎卡片（真实书摘 + 日期 + 问候）；读到最后一章末尾时弹出完成纪念（第 X 本 + 随机寄语 + 累计阅读时长）。完成记录存浏览器 `localStorage`（`lr_completions_v1`），防重复、本地优先、不依赖 Supabase、不进 `data/`。
  - **阅读区「高亮」改为可切换（Toggle）+ 撤销/重做**：选中文本点「高亮」= 添加；再次完整选中同一高亮点「高亮」= 取消（判定基于「章节 + 段落 + 字符偏移」位置，不依赖文字内容，部分覆盖不误删、多段独立）；支持 `Ctrl+Z` 撤销、`Ctrl+Shift+Z` / `Ctrl+Y` 重做，以及刚操作后的底部「撤销」提示。
- **修改功能**
  - `book` 对象新增 `marks` 字段（高亮 / 批注数组），经既有 `PUT /api/books/:id` 整体替换持久化；阅读进度在 `{c,s,u}` 基础上新增 `pid/fp/page` 字段（精准恢复与显示）。
  - 启动脚本 `start.bat` / `start_silent.bat`：系统未安装 Node.js 时自动回退到 WorkBuddy 自带 Node，确保双击 / 开机自启可正常拉起服务（依赖 WorkBuddy 运行环境，无需在系统单独安装 Node.js）。
- **删除功能**：无（PDF 解析能力此前已彻底移除；本期确认上传层仍仅接受 `.txt/.md/.epub` 与粘贴，无 `.pdf`）。
- **Bug 修复**
  - EPUB 上传 "Failed to fetch"：部分 EPUB 以 zlib-wrapped deflate 当 raw deflate 嵌入，浏览器解压失败时自动剥离 zlib 头（78 9C / 78 DA / 78 01）并重试；单章节解压失败跳过该章继续；错误提示区分 "Failed to fetch" 引导硬刷新 / Calibre 重存 / 粘贴。
  - 桌面端重启打不开（系统无 Node.js）：启动脚本增加 WorkBuddy 自带 Node 回退（见上）。
- **数据库变更**：本地 `store.json` 的 `book` 对象新增 `marks`；浏览器 `localStorage` 新增 `lr_completions_v1`（阅读完成记录，非 `store.json`、非云端）；云端 `kb_store` 无变化。
- **API 变更**：无新增端点；高亮 / 批注 `marks` 复用既有 `PUT /api/books/:id`（仅当 `body.marks` 为数组时整体替换，缺失则不改动）。
- **Prompt 变更**：无。
- **文档一致性**：四份文档统一到 v1.3.0；修正此前「阅读区视觉划线高亮已移除」的旧描述（v1.3.0 已重新实现为可切换高亮）；目录职责、页面结构图、功能—模块—API 映射、Roadmap 同步覆盖阅读仪式与高亮 Toggle。
...
- **新增功能**
  - **本地自动备份与恢复（WorkbenchBackup）**：新增服务端备份模块 + 前端「💾 备份与恢复」弹窗。
    ...
    - 手动备份：顶栏「💾 备份与恢复」→「立即备份」。
    - 备份内容：`data/store.json`（**脱敏副本**，剔除 `apiKey/password/token/service_role` 等敏感键）+ `data/books/*.json`（**仅解析后的 JSON，不含原始 EPUB/TXT 等书籍二进制**）。
...
- 📝 **划词批注 / 收藏 / 高亮**：选中文本弹出工具条，可写批注、直接收藏到知识库、或点「高亮」做可切换高亮（再次完整选中同一高亮即可取消，支持 `Ctrl+Z` 撤销 / 重做）
- 🖍 **可切换高亮（Toggle）**：阅读区选中文字即可高亮 / 取消高亮，基于「章节 + 段落 + 字符偏移」精确定位，同一句话多次出现也不会误取消其他位置；多段高亮彼此独立
- 🌿 **阅读欢迎 & 读完纪念**：打开时随机欢迎（真实书摘 + 日期 + 问候）；读完最后一章弹出完成纪念（第 X 本 + 随机寄语 + 累计阅读时长），本地记录防重复
- 🗒 **笔记管理**：随手记录阅读笔记，支持导出
- 🏷 **标签管理**：为词条 / 笔记打标签，灵活归类
- 🗂 **知识库**：词汇 / 表达 / 句型 / 精彩句 / 写作素材 / 文学笔记 / 批注，统一沉淀与检索（删除/清空经墓碑 `deletedKbIds` 跨设备同步，避免云端复活）
- ☁ **可选云端同步**：多设备、手机端实时查看知识库与阅读时长
- 💾 **本地自动备份与恢复**：自动 / 手动备份书库、进度、划线批注、阅读记录、知识库与设置到本地文件夹；可查看历史、恢复指定版本（不含原始书籍二进制、不含密钥、不上云）
- 🔄 **返回原阅读位置**：从知识库 / 批注「定位到原文」后，阅读区顶部出现「↩ 返回阅读位置」按钮，点按精确回到跳转前的位置，且这次临时跳转不覆盖正常阅读进度
...
工作台依赖本地 Node 服务（`server.js`，端口 3007）。以下方案保证开机与休眠恢复后服务可用：

- **开机自启（弹黑框，稳定）**：启动文件夹 `LinguaReader_AutoStart.bat`（路径 `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup`）开机时直接前台运行 WorkBuddy 自带 Node（绝对路径形式的 `%USERPROFILE%\.workbuddy\binaries\node\versions\<版本>\node.exe server.js`），会弹出一个命令框，但服务稳定可用。
- **休眠 / 睡眠恢复自动拉起**：新增任务计划程序任务 `LinguaReader_Service_Restart`，触发器为「工作站解锁时」自动运行 `D:\LinguaReader\restart_lr.bat`，解决笔记本合盖休眠恢复后启动文件夹自启项不重跑、服务进程被系统清理导致的「服务未连接」。
- **一键手动恢复**：`D:\LinguaReader\restart_lr.bat` —— 双击即结束占用 `3007` 的进程并重新启动服务（弹黑框），供服务意外停止时快速救活；也可右键发送到桌面做快捷方式。
- `run_server_hidden.vbs`：无窗口启动器（尝试隐藏 Node 黑框），但当前存在引号拼接 bug 已**暂弃用**，自启回退为上面的弹黑框方案。

> 当前默认方案是「弹黑框自启 + 解锁自动重启」，服务可用优先于无窗口美观。若日后修复无窗口启动器，可在启动文件夹改回调用 `run_server_hidden.vbs`。
...
| 路径 | 用途 | 实际落盘 |
|---|---|---|
| `data/store.json` | **主数据（唯一真相源）**：书籍元数据 + 解析正文 `chapters` + 阅读进度 `progress` + 知识库 `kb` + 批注/划线 + 阅读记录 `reading`/`checkins` + 偏好 `prefs` + `lemmaOverrides` + `deletedBookIds`（书籍墓碑）+ `deletedKbIds`（笔记墓碑） | ✅ 持续写入（原子写 `.tmp`+rename，写前留 `.bak`） |
...
- **安全**：备份全程失败不影响正常使用；备份内容经脱敏（剔除 `apiKey/password/token/service_role` 等敏感键）；原始 EPUB/TXT 等书籍文件不重复复制。
...
> **关于启动脚本 `start.bat` / `start_silent.bat`**：优先从系统 `PATH` 查找 `node`；若本机没装 Node.js，会**自动回退到 WorkBuddy 自带的 Node**（扫描用户目录下 WorkBuddy 运行环境里的最高版本 Node，具体路径见脚本中的回退分支），无需手动安装即可双击 / 开机自启拉起服务。若既无系统 Node 也未安装 WorkBuddy，则提示去 nodejs.org 安装。
...
| `restart_lr.bat` | 一键重启本地服务（结束 3007 占用 + 重新启动 `server.js`），供休眠/重启后手动恢复 |
| `run_server_hidden.vbs` | 无窗口启动器（**暂弃用**：当前引号拼接 bug，自启回退为弹黑框方案） |
| 启动文件夹 `LinguaReader_AutoStart.bat` | 开机自启入口（直接前台运行 WorkBuddy 自带 Node，弹黑框、稳定可用） |
